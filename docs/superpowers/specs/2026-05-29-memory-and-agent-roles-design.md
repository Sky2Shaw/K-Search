...
完整 prompt 草案(`{definition_text}`/`{max_chars}` 为注入占位符,字段可按实测迭代):
...
约束设计:工具仅 `Read/Grep/Glob/Write` + prompt 双重强调只读;明确"不提优化建议"以与 `CodegenAgent`/`world_model` 职责隔离;`EXACTLY this template` 锁定结构保证跨轮/跨算子格式一致;`{max_chars}` 注入护栏防记忆反噬 prompt 预算。
